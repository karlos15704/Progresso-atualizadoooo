...
export async function scanBubbleSheet(
  ...
  for (let qIdx = 0; qIdx < rowCount; qIdx++) {
    const q = correctAnswers[qIdx];
    if (!q) continue;

    const qPoints = parseFloat(String(q.points ?? 1));
    maxPossible += qPoints;

    const isEssay = q.correctAnswer === '__ESSAY__';
    ...
    const currentQ = correctAnswers[qIdx];
    if (currentQ && currentQ.id != null) {
      results[currentQ.id] = bestOption;
    }
    const studentAns = (bestOption || "").toString().trim().toUpperCase();
    const correctAns = currentQ ? (currentQ.correctAnswer || "").toString().trim().toUpperCase() : "";
